...
public class Solution {
    ...
    public static class ReaderThread extends Thread {
        ...
        public void run() {
            try {
                while (!interrupted()) { // пока нить не прервана
                    if (reader.ready()) { // boolean ready() - возвращает значение true, если следующий запрос не будет ожидать.
                        result.add(reader.readLine()); // добавляет в список текст
                        countReadStrings.incrementAndGet(); // счетчик +1;
                    }
                }
            } catch (IOException e) {
                e.getStackTrace();
