...
public class Solution {
    ...
    public static int calculateHorsesFinished(List<Horse> horses) throws InterruptedException {
        int countFinished = 0;
        for (Horse x : horses) { // считываем весь список horses
            if (!x.isFinished()) { // если не финишировал
                System.out.println("Waiting for " + x.getName()); // то выводим номер не винишированной лошоди
                x.join(); // ждем финиша
            }
            else {
                countFinished++; // считаем количество финишированных лошидей
            }
        }
