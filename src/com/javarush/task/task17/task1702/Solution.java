...
public class Solution {
    ...
    public static class SortThread extends Thread { // создаем сласс SortThread и наследуем от Thread
        @Override
        public void run() { // переопределяем метод run
            sort(testArray); // запускаем метод sort и включаем в нее переменную testArray
            //Arrays.sort(testArray); // сортирует в обраную сторону
        }
    }
}
